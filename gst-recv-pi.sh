...
gst-launch-1.0 rtpbin name=rtpbin max-rtcp-rtp-time-diff=50 latency=2000 \
	...
	port=$rtp_port ! rtpbin.recv_rtp_sink_0 \
        rtpbin. ! rtph264depay ! queue flush-on-eos=true max-size-buffers=0 max-size-time=0 max-size-bytes=1004857600 ! h264parse ! mpegtsmux ! filesink buffer-mode=2 location=gst-omx-pipe  \
