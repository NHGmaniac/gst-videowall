...
current_time=$( date +%s )
if (( file_time < ( current_time - ( 24 * 60 * 60) ) )); then
    ...
    until sh -c "TERM=linux apt install -y bc git gstreamer1.0-plugins-base gstreamer1.0-plugins-bad gstreamer1.0-tools omxplayer gstreamer1.0-plugins-good figlet dnsutils python3-gst-1.0 > /dev/tty0"; do
...
while true; do 
	until curl -f http://videowall.nhg.moe:8082/$mac/$(hostname -I | awk '{print $1}') -o /etc/gst-videowall/config; do
	    mac=$(cat /sys/class/net/$(ip route show default | awk '/default/ {print $5}')/address)
